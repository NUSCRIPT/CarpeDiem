...
class Sankey:
    def __init__(self, x, y,
        ...
        self.init_nodes(left_order, right_order)
    ...
    def init_nodes(self, left_order, right_order):
        left_nodes = {}
        right_nodes = {}
        left_offset = 0
        ...
        if right_order is None:
            ...
            key = lambda pair: right_order.index(pair[0])
            for name, node in sorted(right_nodes.items(), key=key):
                self.right_nodes[name] = node[0]
                self.right_nodes_idx[name] = len(self.right_nodes_idx)
